...
func (s *Store) Snapshot() (list []MemberEntry, selfID nodeid.NodeID) {
	s.mu.Lock()
	defer s.mu.Unlock()
	return s.SnapshotWithoutLock()
}

func (s *Store) SnapshotWithoutLock() (list []MemberEntry, selfID nodeid.NodeID) {
	entryMap := s.entries
...
//   - Prefer newer boot (new boot eclipses old).
...
func (s *Store) apply(incomingNodeID nodeid.NodeID, incomingIncNumber uint64, state State, updateVersion bool) {
	...
	if entry, ok := s.entries[lookupKey]; ok {
		...
		if updateVersion {
			...
			switch state {
			case StateSuspect:
				log.Printf(generic_utils.Orange+"Apply: %s updated existing entry: %+v\n"+generic_utils.Reset, state.StateToString(), *entry)
			case StateFailed:
				log.Printf(generic_utils.Red+"Apply: %s updated existing entry: %+v\n"+generic_utils.Reset, state.StateToString(), *entry)
			case StateLeft:
				// Update ring when a member voluntarily leaves
				// list, _ := s.SnapshotWithoutLock()
				// ring.UpdateRingFromMembership(list)
				s.notifyLocked("Member Left: updating ring")

				log.Printf(generic_utils.Cyan+"Apply: %s updated existing entry: %+v\n"+generic_utils.Reset, state.StateToString(), *entry)
			default:
				log.Printf(generic_utils.Underline+"Apply: %s updated existing entry: %+v\n"+generic_utils.Reset, state.StateToString(), *entry)
			}
		}
	} else {
...
func (s *Store) RemoveEntry(incomingNodeID nodeid.NodeID) {
	...
	if entry, ok := s.entries[lookupKey]; ok {
		...
		s.version++
		log.Print(generic_utils.DarkRed+("Entry deleted: "), *entry, "\n"+generic_utils.Reset)
		s.PrintSnapshot()

		// Update ring on member removal
		// list, _ := s.SnapshotWithoutLock()
		// ring.UpdateRingFromMembership(list)
		s.notifyLocked("Member Removed: updating ring")
	} else {
		log.Print("Entry not found \n")
	}
}
...
// Keep the newer boot and drop the older.
...
func (s *Store) selfBumpIncarnation() {
	...
	if entry, ok := s.entries[lookupKey]; ok {
		...
		s.version++
		log.Print((generic_utils.Pink + "SelfBumpIncarnation: new incarnation: "), entry, "\n"+generic_utils.Reset)
	} else {
...
func (s *Store) updateMemberEntry(incomingMemberEntry MemberEntry) {
	...
	if existingEntry, ok := s.entries[lookupKey]; ok {
		...
		log.Print(generic_utils.Yellow+"New entry, adding to membership list", incomingMemberEntry, "\n"+generic_utils.Reset)
		// incomingMemberEntry.LastUpdate = time.Now()
		s.entries[lookupKey] = &incomingMemberEntry
		s.version++
		s.PrintSnapshot()

		// Update ring on new member addition
		// list, _ := s.SnapshotWithoutLock()
		// ring.UpdateRingFromMembership(list)
		s.notifyLocked("New Member Added: updating ring")
		return
	}
}
...
func (s *Store) PrintTable(tag string) {
	...
	//list := membershipList
	//log.Printf(generic_utils.Purple + "-----------------------------------------------------------------------")
...
func (s *Store) PrintSnapshot() {
	...
	for _, m := range membershipList {
		log.Printf("  %s  inc=%d  state=%v  at=%s\n",
			generic_utils.ResolveDNSFromIP(m.ID.NodeIDToString()), m.Incarnation, m.State, m.LastUpdate.Format("2006-01-02 15:04:05"))
	}
...
func (s *Store) PrintStateChange(prev, next MemberEntry) {
	if prev.State != next.State {
		switch next.State {
		case StateSuspect:
			log.Printf(generic_utils.Orange+"Apply: %s updated existing entry: %+v\n"+generic_utils.Reset, next.State.StateToString(), next)
		case StateFailed:
			log.Printf(generic_utils.Red+"Apply: %s updated existing entry: %+v\n"+generic_utils.Reset, next.State.StateToString(), next)
		case StateLeft:
			log.Printf(generic_utils.Cyan+"Apply: %s updated existing entry: %+v\n"+generic_utils.Reset, next.State.StateToString(), next)
