...
func (rm *ReplicaManager) localPlanJoin(lp *LocalPlan, self Node, newRing *Ring, joinedNode Node) {
	...
	log.Print("Calculating join plan for node: ", joinedNode.NodeID.NodeIDToString(), "\n")
...
func (rm *ReplicaManager) ExecuteLocal(mover *HTTPMover, lp LocalPlan) error {
	...
	log.Println("Re-replication time:", elapsed)
	log.Println("Total bytes sent:", totalBytesSent)
...
func (m *HTTPMover) resolveReplicaEndpoint(rp nodeid.NodeID) (string, error) {
	...
	log.Println("Resolving replica endpoint for:", replicaEndpoint)
	...
	log.Println("Host extracted:", host)
	...
	if m.Config.Env == "dev" {
		...
		log.Println("Calculated hydfsHTTPPort:", hydfsHTTPPort)
	...
	log.Println("Resolved replica endpoint to:", replicaEndpoint)
