...
func (fs *FileStore) Create(fileName string, r io.Reader, clientID string, clientSeq uint64, ts time.Time) (*FileOpResult, error) {
	...
	log.Printf("[file_store] create successful: %s\n", fileTokenStr)
	...
	log.Printf("[file_store] create successful: %s (version: %d, bytes: %d)\n", fileName, res.Version, res.Bytes)
	return res, nil
}
...
func (fs *FileStore) Append(fileName string, r io.Reader, clientID string, clientSeq uint64, ts time.Time) (*FileOpResult, error) {
	fileToken := ids.FileToken64(fileName)
	fileTokenStr := strconv.FormatUint(fileToken, 10)
	log.Printf("[file_store] append received: %s (client: %s)\n", fileTokenStr, clientID)

	// ensure dirs exist
	ok, err := fs.Paths.FileDirsExist(fileToken)
	if err != nil {
		log.Printf("[file_store] file dir does not exist: %s\n", fileTokenStr)
		return nil, os.ErrNotExist
	}
	if !ok {
		log.Printf("[file_store] file does not exist: %s\n", fileTokenStr)
		return nil, os.ErrNotExist
	}

	manifestPath := fs.Paths.ManifestPath(fileToken)
	if _, err := os.Stat(manifestPath); err != nil { // check if manifest already exists
		log.Printf("[file_store] manifest file doesn't exist: %s\n", fileTokenStr)
		return nil, os.ErrNotExist
	}
	...
	log.Printf("[file_store] append successful: %s (version: %d, bytes: %d)\n", fileName, m.Version, total)
...
func (fs *FileStore) GetFile(fileName string, w io.Writer) (*GetResult, error) {
	log.Printf("[file_store] get received: %s\n", fileName)
	fileToken := ids.FileToken64(fileName)
	fileTokenStr := strconv.FormatUint(fileToken, 10)
	...
	manifestPath := fs.Paths.ManifestPath(fileToken) // path to manifest.json for this fileToken
	if _, err := os.Stat(manifestPath); err != nil { // check if manifest already exists
		log.Printf("[file_store] manifest file does not exist: %s\n", fileTokenStr)
		return nil, os.ErrNotExist
	}
	...
	log.Printf("[file_store] get successful: %s (version: %d, bytes: %d)\n", fileName, m.Version, total)
...
func (fs *FileStore) GetManifest(fileName string) (*Manifest, error) {
	fileToken := ids.FileToken64(fileName)
	log.Printf("[file_store] get manifest received: %s\n", fileName)
	...
	log.Printf("[file_store] get manifest successful: %s (version: %d)\n", fileName, m.Version)
	return m, nil
}

// storage/file_store_local.go
func (fs *FileStore) CreateLocalFile(localFileName string, r io.Reader) error {
	log.Printf("[file_store] create local file received: %s\n", localFileName)
	...
	log.Printf("[file_store] create local file successful: %s (bytes: %d)\n", localFileName, n)
	return err
}
