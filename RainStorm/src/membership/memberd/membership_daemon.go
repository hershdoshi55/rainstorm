...
import (
	...
	"rainstorm-c7/membership/protocol/common"
	"rainstorm-c7/membership/protocol/gossip"
	"rainstorm-c7/membership/protocol/pingack"
	"rainstorm-c7/membership/store"
	"rainstorm-c7/membership/suspicion"
	"rainstorm-c7/membership/transport"
	"rainstorm-c7/membership/wire"
...
func Run(cfg config.Config, st *store.Store, selfNodeID nodeid.NodeID) (*MembershipDaemon, error) {
...
func (d *MembershipDaemon) startProtocol(cfg config.Config) {
	switch cfg.Mode {
...
func (d *MembershipDaemon) stopPingAck() {
	if d.loopPA != nil {
		d.loopPA.Close()
		d.loopPA = nil
	}
...
func (d *MembershipDaemon) stopGossip() {
	if d.loopGo != nil {
		d.loopGo.Close()
		d.loopGo = nil
	}
...
func (d *MembershipDaemon) applyRuntime(prev, next config.Config) {
...
func (d *MembershipDaemon) onConfigFromWire(dto config.ConfigDTO) {
	d.cfgMu.Lock()
	prev := d.cfg
	...
	log.Printf(generic_utils.Cyan+"received config change from wire: version %d"+generic_utils.Reset, next.Version)
	d.applyRuntime(prev, next)
}

func (d *MembershipDaemon) membershipHTTP(addr string) {
	...
	http.HandleFunc("/set", func(w http.ResponseWriter, r *http.Request) {
		...
		if changed {
			log.Printf(generic_utils.Cyan+"received config change from HTTPServer: version %d"+generic_utils.Reset, next.Version)
			d.applyRuntime(prev, next)
		}
	...
	http.HandleFunc("/switch", func(w http.ResponseWriter, r *http.Request) {
		...
		if changed {
			log.Printf(generic_utils.Cyan+"switch: protocol=%s suspicion=%s -> version %d"+generic_utils.Reset, dto.Mode, s, next.Version)
			d.applyRuntime(prev, next)
		}
...
func (d *MembershipDaemon) startIntroducerServer(addr string) {
...
func (d *MembershipDaemon) handleJoin(conn net.Conn) {
	defer conn.Close()
...
func (d *MembershipDaemon) joinCluster(introducerAddr string) {
	for {
		...
		if changed {
			log.Printf(generic_utils.Cyan+"received config change from Introducer: version %d"+generic_utils.Reset, next.Version)
			d.applyRuntime(prev, next)
		}
...
func (d *MembershipDaemon) Close() {
	...
	fmt.Println("membership daemon closed")
}
