...
import (
	...
	"os/signal"
	"rainstorm-c7/config"
	hydfs_daemon "rainstorm-c7/hydfs/cmd/hydfsd"
	membership_daemon "rainstorm-c7/membership/memberd"
	nodeid "rainstorm-c7/membership/node"
	"rainstorm-c7/membership/store"
	rainstorm_daemon "rainstorm-c7/rainstorm/cmd/rainstormd"
	"strings"
	"syscall"
)
...
func resolveAddrPortUDP(s string) (netip.AddrPort, error) {
	...
	ip = ip.Unmap() // convert ::ffff:x.x.x.x (IPv6-mapped IPv4) to plain IPv4
...
func main() {
	...
	// Start Rainstorm Daemon
	rainstormd, err := rainstorm_daemon.Run(cfg, st, selfNodeID)
	if err != nil {
		log.Fatal(err)
	}
	defer rainstormd.Close()
	...
	memberd, err := membership_daemon.Run(cfg, st, selfNodeID)
	if err != nil {
		log.Fatal(err)
	}
	defer memberd.Close()
